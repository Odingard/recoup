...
The fee applies to dollars backed by realization events and, for legacy
findings without events, to lifecycle ``recovered`` dollars. Proposed and
approved dollars are never billed; only realized dollars are.
"""
...
def compute_metrics(findings: list[dict], events: list[dict] | None = None,
    ...
    if events is None:
        ...
        recovered_ids = {f.get("finding_id") for f in recovered}
        recovered_this_month = sum(
            -(e.get("reversal_amount") or 0)
            if e.get("event_type") == "reversal"
            else (e.get("realized_value") or 0)
            for e in events
            if e.get("finding_id") in recovered_ids
            and _month(e.get("realized_at") or e.get("created_at"))
            == current_month
        )
        recovered_count = sum(1 for f in recovered
                              if _recovered_dollars(f) > 0)
